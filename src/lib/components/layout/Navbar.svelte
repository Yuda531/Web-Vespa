...
>
  ...
  >
    ...
    <div class="hidden items-center gap-1 md:flex">
      {#each NAV_LINKS as link}
        <a
          href={link.href}
          class={cn(
            "rounded-(--radius) px-3 py-2 text-sm font-medium transition-colors hover:bg-(--secondary) hover:text-(--foreground)",
    ...
    <!-- Mobile toggle -->
    <button
      class="inline-flex items-center justify-center rounded-(--radius) p-2 text-(--muted-foreground) hover:bg-(--secondary) md:hidden"
      ...
      <div class="space-y-1 px-4 pb-4 pt-2">
        {#each NAV_LINKS as link}
          <a
            ...
            onclick={closeMobile}
            class={cn(
              "block rounded-(--radius) px-3 py-2 text-sm font-medium transition-colors hover:bg-(--secondary)",
              $page.url.pathname === link.href ||
                ($page.url.pathname.startsWith(link.href) && link.href !== "/")
                ? "bg-(--secondary) text-(--foreground)"
                : "text-(--muted-foreground)",
